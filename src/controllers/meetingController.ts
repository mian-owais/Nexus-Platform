    ...
    if (conflicts.length > 0) {
      return {
        ...
        conflicts: conflicts.map(c => ({
          ...
          endTime: c.endTime,
          affectedParticipants: c.participantIds.filter(p =>
...
export const acceptMeeting = async (
  ...
  try {
    ...
    const participantResponse = meeting.participantResponses.find(
    ...
    const allAccepted = meeting.participantResponses.every(
      pr => pr.response === 'accepted'
    );
  ...
  try {
    ...
    if (startTime || endTime) {
      ...
      const conflictCheck = await checkConflicts(
        newStartTime,
        newEndTime,
        meeting.participantIds.map(p => p.toString()),
        meetingId
      );
...
export const getAvailableSlots = async (
  ...
  try {
    ...
    const meetingDuration = parseInt(duration as string) || 30; // minutes
    ...
    while (currentTime.getHours() < workingHourEnd) {
      ...
      const isConflict = meetings.some(
        m => currentTime < m.endTime && slotEnd > m.startTime
      );
